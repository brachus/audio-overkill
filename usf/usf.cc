...
void usf_fread(void *ptr, int64_t size, int64_t nmemb, struct filebuf * file)
{   
	int ret;
	ret = filebuf_fread(ptr, size, nmemb, file);
    if (ret != nmemb)
		printf
...
void usf_fseek(struct filebuf * file, int64_t offset, int whence)
{
	int ret;
	ret = filebuf_fseek(file, offset, whence);
    if (!ret)
...
int LoadUSF(const char * fn, struct filebuf * fil)
{
    ...
    reservedsize = FROM_LE32(((uint32_t *) fil->buf)[1]);
    codesize = FROM_LE32(((uint32_t *) fil->buf)[2]);
    crc = FROM_LE32(((uint32_t *) fil->buf)[3]);
    

    usf_fseek(fil, 0, _AO_FBUF_END);
    ...
    if (tagsize)
    {
		...
		strcpy(tag_system, "Nintendo 64");

		usftag_raw_getvar((char *) tagbuffer, "_enablecompare",
				  (char *) buffer2, 50000);
		if (strlen((char *) buffer2))
			enablecompare = 1;
		else
		...
		usftag_raw_getvar((char *) tagbuffer, "_enableFIFOfull",
				  (char *) buffer2, 50000);
		if (strlen((char *) buffer2))
			enableFIFOfull = 1;
		else
		...
		usftag_raw_getvar((char *) tagbuffer, "length", (char *) buffer2,
				  50000);
		if (strlen((char *) buffer2))
			track_time = get_length_from_string(buffer2);

		usftag_raw_getvar((char *) tagbuffer, "fade", (char *) buffer2,
				  50000);
		if (strlen((char *) buffer2))
			fade_time = get_length_from_string(buffer2);
		
		/* scrape game tag */
		usftag_raw_getvar((char *) tagbuffer, "game", (char *) buffer2,
				  50000);
		if (strlen((char *) buffer2))
			strcpy(tag_game, buffer2); /* FOR AO.H */
		
		/* scrape author tag */
		usftag_raw_getvar((char *) tagbuffer, "artist", (char *) buffer2,
				  50000);
		if (strlen((char *) buffer2))
			strcpy(tag_author, buffer2); /* FOR AO.H */
		
		/* scrape year tag */
		usftag_raw_getvar((char *) tagbuffer, "year", (char *) buffer2,
				  50000);
		if (strlen((char *) buffer2))
			strcpy(tag_year, buffer2); /* FOR AO.H */
		
		/* scrape title tag */
		usftag_raw_getvar((char *) tagbuffer, "title", (char *) buffer2,
				  50000);
		if (strlen((char *) buffer2))
		{
    ...
    usf_fseek(fil, reservestart, _AO_FBUF_SET);
    usf_fread(&temp, 4, 1, fil);

    if (temp == 0x34365253)
    {	//there is a rom section
		int len = 0, start = 0;
...
bool usf_play(const char * fn, struct filebuf* file)
{
	...
	*/
	...
	/*StartEmulationFromSave(savestatespace); HERE */
...
/* REDO this for tag strings in ao.h */
/*
